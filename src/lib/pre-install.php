...
file_put_contents($HTTPD_CONF, '
  ...
  RewriteRule ^' . $MOUNT_PATH . '$ /monitoring/helicorders/index.php [L,PT]

  # Pretty URLs
  RewriteRule ^' . $MOUNT_PATH . '/([0-9]+)/([0-9]{8}|latest)$ ' .
    $MOUNT_PATH . '/seismogram.php?id=$1&date=$2 [L,PT]
  RewriteRule ^' . $MOUNT_PATH . '/([0-9]{8}|latest)$ ' .
    $MOUNT_PATH . '/seismograms.php?date=$1 [L,PT]
  RewriteRule ^' . $MOUNT_PATH . '/([0-9]+)$ ' .
    $MOUNT_PATH . '/seismograms.php?id=$1 [L,PT]
